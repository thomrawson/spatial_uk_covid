...
n_chains: %s \n
  ...
  rw_penalty: %s ", tree_depth, n_chains, 
...
if(scale_by_susceptible_pool){
  main_summaries <- summary(stanfit, pars = c('sqrtQ', 'susc_scaling', 'beta_random_walk_steps', 'zetas', 'phi', 'lp__'))
  write.csv(main_summaries$summary,"Outputs/main_summaries.csv", row.names = TRUE)

}else{
main_summaries <- summary(stanfit, pars = c('sqrtQ', 'beta_random_walk_steps', 'zetas', 'phi', 'lp__'))
write.csv(main_summaries$summary,"Outputs/main_summaries.csv", row.names = TRUE)
}
...
#E is the "current" weeks cases.  i.e. E[2] = y[1]
  ...
  for(i in 1:T){
    if(scale_by_susceptible_pool){
      model_approx_y[i,] <- as.numeric(((model_susc_scale*susceptible_proxy[,i])*(E[i,] + (model_zetas*E_neighbours[,i]))))*exp( (model_beta_random_walk[i]) + model_theta)
      
    }else{
      model_approx_y[i,] <- as.numeric((susceptible_proxy[,i]*(E[i,] + (model_zetas*E_neighbours[,i]))))*exp( (model_beta_random_walk[i]) + model_theta)
    }
  }
...
########################
  ...
  add_traject <- rstan::traceplot(stanfit, pars=c(sprintf('theta[%s]',301:306),
                                                  sprintf('beta_random_walk_steps[%s]',91:95)), nrow = 2)
  new_names <- c(sprintf('theta[%s]',301:306),
                 sprintf('X[%s]',91:95))
  ...
  png(file="Outputs\\boxplot_theta.png",
      width=1280, height=960, res = 150)
  boxplot(list(theta = theta_means), 
          main = "Mean values for theta across 306 LTLAs",
          ylab = "Values",
