...
K <- 25 #; Was 26 but changed to 21 for sgtf variants
#K <- 21
...
for(i in 2:final_week){
  ...
  x[j,,20] <- scale(Reduced_Data$Core_services_funding_by_weighted)
  x[j,,21] <- scale(Reduced_Data$Primary_care_funding_by_weighted)
  x[j,,22] <- scale(Reduced_Data$Specialised_services_by_weighted)
  x[j,,23] <- scale(Reduced_Data$unringfenced/Reduced_Data$Population)
  x[j,,24] <- scale(Reduced_Data$contain_outbreak_management/Reduced_Data$Population)
  x[j,,25] <- scale(Reduced_Data$ASC_infection_control_fund/Reduced_Data$Population)
