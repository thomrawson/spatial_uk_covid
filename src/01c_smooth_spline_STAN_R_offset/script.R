...
#instead off-setting with population, we offset with LAST WEEK's cases!
...
dir.create("outputs/plots/R_est")
dir.create("outputs/plots/Cases_by_population")
dir.create("outputs/plots/R_est_via_smooth_spline")
...
sp_check_10 <- gam(Week_Cases ~  s(y_scale, x_scale, k = 10, bs = "tp"), offset = log(previous_week_cases),
...
plot_data <- Reduced_Data[,c(1,4,8,10)]
...
Boundaries %>% 
  ...
  theme_void() +
  theme(plot.background = element_rect(fill = 'white', color = "white")) -> R_plot

ggsave(R_plot, 
       filename = sprintf("outputs/plots/R_est/Week_%s_R_est.png", Week_isolated))
...
Boundaries %>% 
  inner_join(plot_data, by = "CODE") %>% 
  ggplot( ) +
  geom_sf(aes(fill = GAM_fit/Previous_Week_Cases), lwd =  .05) +
  scale_fill_viridis_c(name = sprintf("Week %s GAM k=10 \nR est.",Week_isolated)) +
  theme_void() +
  theme(plot.background = element_rect(fill = 'white', color = "white")) -> Gam_k10_plot

ggsave(Gam_k10_plot, 
       filename = sprintf("outputs/plots/R_est_via_smooth_spline/Week_%s_k_10.png", Week_isolated))



#What if I give the smoothing spline more basis functions to work with?
sp_check_20 <- gam(Week_Cases ~  s(y_scale, x_scale, k = 20, bs = "tp"), offset = log(previous_week_cases),
                   family = "poisson", data = Reduced_Data, method = "REML")
...
Boundaries %>% 
  inner_join(plot_data, by = "CODE") %>% 
  ggplot( ) +
  geom_sf(aes(fill = GAM_fit_20/Previous_Week_Cases), lwd =  .05) +
  scale_fill_viridis_c(name = sprintf("Week %s GAM k=20 \nR est.",Week_isolated)) +
  theme_void() +
  theme(plot.background = element_rect(fill = 'white', color = "white")) -> Gam_k20_plot

ggsave(Gam_k20_plot, 
       filename = sprintf("outputs/plots/R_est_via_smooth_spline/Week_%s_k_20.png", Week_isolated))
...
#anova(sp_check_10, sp_check_20, test = "Chisq")

#HUGELY so. What about 40 basis functions?

sp_check_40 <- gam(Week_Cases ~  s(y_scale, x_scale, k = 40, bs = "tp"), offset = log(previous_week_cases),
                   family = "poisson", data = Reduced_Data, method = "REML")
...
Boundaries %>% 
  inner_join(plot_data, by = "CODE") %>% 
  ggplot( ) +
  geom_sf(aes(fill = GAM_fit_40/Previous_Week_Cases), lwd =  .05) +
  scale_fill_viridis_c(name = sprintf("Week %s GAM k=40 \nR est.",Week_isolated)) +
  theme_void() +
  theme(plot.background = element_rect(fill = 'white', color = "white")) -> Gam_k40_plot

ggsave(Gam_k40_plot, 
       filename = sprintf("outputs/plots/R_est_via_smooth_spline/Week_%s_k_40.png", Week_isolated))
...
data {
  ...
  int<lower=0> e[N];              // offset (previous week's cases)
...
modelData <- list(N = length(Reduced_Data$Week_Cases), M = ncol(X),
                  y = Reduced_Data$Week_Cases, X = X, zero = jd_smooth$jags.data$zero, 
                   S1 = jd_smooth$jags.data$S1, e = Reduced_Data$previous_week_cases)
...
#5mins to run iter = 2000 (others default)
...
colnames(plot_data) <- c("CODE", "Population", "Week_Cases", "Previous_Week_Cases", "u_mean", "v_mean")
...
plot_grid(R_plot + theme(legend.title= element_blank()),
          Gam_k40_plot + theme(legend.title= element_blank()), uplot, vplot,
          nrow = 2, labels = c(sprintf("Real R est. - Week %s", Week_isolated), "GAM k40 fit",
