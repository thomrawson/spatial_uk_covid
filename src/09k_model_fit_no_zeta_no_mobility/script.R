...
y[,1] ~ poisson_log(log((susc_scaling*susceptible_proxy[,1]).*(E[,1])) + x[1] * betas + (beta_random_walk[1]) + theta);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ poisson_log(log((susc_scaling*susceptible_proxy[,i]).*(E[,i])) + x[i] * betas + (beta_random_walk[i]) + theta);  // extra noise removed removed: + theta[,i]
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = poisson_log_lpmf(y[n, t] | log((susc_scaling*susceptible_proxy[n,t]) * (E[n,t])) + dot_product(x[t,n,], betas) + beta_random_walk[t] + theta[n]);
    }                                               
  }
}
"
...
y[,1] ~ poisson_log(log((susceptible_proxy[,1]).*(E[,1])) + x[1] * betas + (beta_random_walk[1]) + theta);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ poisson_log(log((susceptible_proxy[,i]).*(E[,i])) + x[i] * betas + (beta_random_walk[i]) + theta);  // extra noise removed removed: + theta[,i]
  target += -penalty_term*fabs(beta_random_walk[i]);
}
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = poisson_log_lpmf(y[n, t] | log((susceptible_proxy[n,t]) * (E[n,t])) + dot_product(x[t,n,], betas) + beta_random_walk[t] + theta[n]);
    }                                               
  }
}
"
