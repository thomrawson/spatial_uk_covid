...
y[,1] ~ neg_binomial_2( ((susc_scaling*susceptible_proxy[,1]).*(E[,1] + (zetas .*E_neighbours[,1]))).*exp( (beta_random_walk[1]) + theta), phi);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ neg_binomial_2( ((susc_scaling*susceptible_proxy[,i]).*(E[,i] + (zetas .*E_neighbours[,i]))).*exp( (beta_random_walk[i]) + theta), phi);  // extra noise removed removed: + theta[,i]
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = neg_binomial_2_lpmf(y[n, t] | ((susc_scaling*susceptible_proxy[n,t]) * (E[n,t]+ (zetas[n]*E_neighbours[n,t])))*exp( beta_random_walk[t] + theta[n]), phi);
    }                                               
  }
}
"
...
y[,1] ~ neg_binomial_2( ((susceptible_proxy[,1]).*(E[,1] + (zetas .*E_neighbours[,1]))).*exp( (beta_random_walk[1]) + theta), phi);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ neg_binomial_2( ((susceptible_proxy[,i]).*(E[,i] + (zetas .*E_neighbours[,i]))).*exp( (beta_random_walk[i]) + theta), phi);  // extra noise removed removed: + theta[,i]
  target += -penalty_term*fabs(beta_random_walk[i]);
}
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = neg_binomial_2_lpmf(y[n, t] | ((susceptible_proxy[n,t]) * (E[n,t]+ (zetas[n]*E_neighbours[n,t])))*exp(beta_random_walk[t] + theta[n]), phi);
    }                                               
  }
}
"
