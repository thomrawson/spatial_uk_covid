...
#E is the "current" weeks cases.  i.e. E[2] = y[1]
  ...
  for(i in 1:T){
    if(scale_by_susceptible_pool){
      model_approx_y[i,] <- as.numeric(((model_susc_scale*susceptible_proxy[,i])*(E[i,])))*exp( (model_beta_random_walk[i]) + model_theta)
      
    }else{
  model_approx_y[i,] <- as.numeric((susceptible_proxy[,i]*(E[i,])))*exp( (model_beta_random_walk[i]) + model_theta)
    }
  }
