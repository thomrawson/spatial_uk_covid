...
zeta_trajectories <- rstan::traceplot(stanfit, pars=c('zetas'))
png(file="Case_Outputs\\zetas_trajectories.png",
    width=1440, height=1080, res = 150)
...
#E is the "current" weeks cases.  i.e. E[2] = y[1]
  ...
  for(i in 1:T){
    if(scale_by_susceptible_pool){
      model_approx_y[i,] <- as.numeric(log((model_susc_scale*susceptible_proxy[,i])*(E[i,] + (model_zetas*E_neighbours[,i])))) + x[i,,]%*%model_betas + (model_beta_random_walk[i])
      
    }else{
  model_approx_y[i,] <- as.numeric(log(susceptible_proxy[,i]*(E[i,] + (model_zetas*E_neighbours[,i])))) + x[i,,]%*%model_betas + (model_beta_random_walk[i])
    }
  }
