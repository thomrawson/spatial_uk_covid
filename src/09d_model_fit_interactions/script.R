...
#The big difference between this and standard task 09 is:
#prop_o65 removed
#Cut workplace and transit movement scores, as they were correlated anyway
#The following interaction terms have been added:
#IMD x unringfenced
#IMD x contain outbreak management fund
#IMD x ASC infection
#IMD x mobility
#mobility x unringfenced
...
data {
  ...
  int<lower=1> K_interaction;                 // num interaction effects
  matrix[N, K_interaction] x_interaction[T];                 // interaction effects matrix
...
parameters {
  ...
  vector[K_interaction] betas_interaction;       // interaction coefficients
...
y[,1] ~ poisson_log(log((susc_scaling*susceptible_proxy[,1]).*(E[,1] + (zetas .*E_neighbours[,1]))) + (x[1] * betas) + (x_interaction[1] * betas_interaction) + (beta_random_walk[1]) + theta);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ poisson_log(log((susc_scaling*susceptible_proxy[,i]).*(E[,i] + (zetas .*E_neighbours[,i]))) + (x[i] * betas) + (x_interaction[i] * betas_interaction) + (beta_random_walk[i]) + theta);  // extra noise removed removed: + theta[,i]
  target += -penalty_term*fabs(beta_random_walk[i]);
}

  betas ~ normal(0.0, 1.0);
  betas_interaction ~ normal(0.0, 1.0);
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = poisson_log_lpmf(y[n, t] | log((susc_scaling*susceptible_proxy[n,t]) * (E[n,t]+ (zetas[n]*E_neighbours[n,t]))) + dot_product(x[t,n,], betas) + dot_product(x_interaction[t,n,], betas_interaction) + beta_random_walk[t] + theta[n]);
    }                                               
  }
}
"
...
y[,1] ~ poisson_log(log((susceptible_proxy[,1]).*(E[,1] + (zetas .*E_neighbours[,1]))) + (x[1] * betas) + (x_interaction[1] * betas_interaction) + (beta_random_walk[1]) + theta);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ poisson_log(log((susceptible_proxy[,i]).*(E[,i] + (zetas .*E_neighbours[,i]))) + (x[i] * betas) + (x_interaction[i] * betas_interaction) + (beta_random_walk[i]) + theta);  // extra noise removed removed: + theta[,i]
  target += -penalty_term*fabs(beta_random_walk[i]);
}
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = poisson_log_lpmf(y[n, t] | log((susceptible_proxy[n,t]) * (E[n,t]+ (zetas[n]*E_neighbours[n,t]))) + dot_product(x[t,n,], betas) + dot_product(x_interaction[t,n,], betas_interaction) + beta_random_walk[t] + theta[n]);
    }                                               
  }
}
"
...
#then cut transit and workplace movement, and prop_o65, back to 13
K <- 13
K_interaction <- 5
x <- array(0, dim = c(T, N,K))
x_interaction <- array(0, dim = c(T, N,K_interaction))
...
for(i in 2:final_week){
  ...
  x[j,,5] <- (Reduced_Data$Pop_per_km2 - mean(Case_Rates_Data$Pop_per_km2))/sd(Case_Rates_Data$Pop_per_km2) 
  x[j,,6] <- (Reduced_Data$Median_annual_income - mean(Case_Rates_Data$Median_annual_income))/sd(Case_Rates_Data$Median_annual_income) 
  ...
  x[j,,7] <- (Reduced_Data$residential_percent_change_from_baseline - mean(Case_Rates_Data$residential_percent_change_from_baseline))/sd(Case_Rates_Data$residential_percent_change_from_baseline)
  ...
  if(use_SGTF_data){
  x[j,,8] <- Reduced_Data$s_Alpha_prop
  x[j,,9] <- Reduced_Data$s_Delta_prop
  x[j,,10] <- Reduced_Data$s_Omicron_prop

  } else{
    x[j,,8] <- Reduced_Data$Alpha_proportion/100
    x[j,,9] <- (Reduced_Data$Delta_proportion + Reduced_Data$Delta_AY_4_2_proportion)/100
    x[j,,10] <- (Reduced_Data$Omicron_BA_1_proportion + Reduced_Data$Omicron_BA_2_proportion + Reduced_Data$Omicron_BA_4_proportion + Reduced_Data$Omicron_BA_5_proportion)/100
  }
  
  
  x[j,,11] <- ((Reduced_Data$unringfenced/Reduced_Data$Population) - mean(Case_Rates_Data$unringfenced/Case_Rates_Data$Population))/sd(Case_Rates_Data$unringfenced/Case_Rates_Data$Population)
  x[j,,12] <- ((Reduced_Data$contain_outbreak_management/Reduced_Data$Population) - mean(Case_Rates_Data$contain_outbreak_management/Case_Rates_Data$Population))/sd(Case_Rates_Data$contain_outbreak_management/Case_Rates_Data$Population)
  x[j,,13] <- ((Reduced_Data$ASC_infection_control_fund/Reduced_Data$Population) - mean(Case_Rates_Data$ASC_infection_control_fund/Case_Rates_Data$Population))/sd(Case_Rates_Data$ASC_infection_control_fund/Case_Rates_Data$Population)
  
  
  #Interaction terms
  x_interaction[j,,1] <- x[j,,4]*x[j,,7] #IMD x mobility
  x_interaction[j,,2] <- x[j,,4]*x[j,,11] #IMD x unringfenced
  x_interaction[j,,3] <- x[j,,4]*x[j,,12] #IMD x contain outbreak management
  x_interaction[j,,4] <- x[j,,4]*x[j,,13] #IMD x ASC infection control
  x_interaction[j,,5] <- x[j,,7]*x[j,,12] #mobility x contain outbreak management
...
stanfit = stan(model_code = Stan_model_string_neighbours,
               data=list(N=N,T=T,
                         y=t(y),
                         x=x, x_interaction = x_interaction,
                         K=K, K_interaction = K_interaction,
                         E=t(E),
