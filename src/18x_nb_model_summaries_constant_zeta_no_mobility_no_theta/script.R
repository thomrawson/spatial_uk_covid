...
#E is the "current" weeks cases.  i.e. E[2] = y[1]
  ...
  for(i in 1:T){
    if(scale_by_susceptible_pool){
      model_approx_y[i,] <- as.numeric(((model_susc_scale*susceptible_proxy[,i])*(E[i,] + (model_zetas*E_neighbours[,i]))))*exp( x[i,,]%*%model_betas + (model_beta_random_walk[i]))
      
    }else{
  model_approx_y[i,] <- as.numeric((susceptible_proxy[,i]*(E[i,] + (model_zetas*E_neighbours[,i]))))*exp( x[i,,]%*%model_betas + (model_beta_random_walk[i]))
    }
  }
