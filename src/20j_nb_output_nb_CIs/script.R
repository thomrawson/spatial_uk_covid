...
if(Preload_data){
  ...
  for(j in 1:n_draws){
    ...
    for(i in 1:T){
      if(scale_by_susceptible_pool){
        y_approx[i,,j] <- as.numeric(((model_susc_scale*susceptible_proxy[,i])*(E[i,])))*exp( (list_of_draws$beta_random_walk[j,i]) + list_of_draws$theta[j,])
        
      }else{
        y_approx[i,,j] <- as.numeric((susceptible_proxy[,i]*(E[i,])))*exp((list_of_draws$beta_random_walk[j,i]) + list_of_draws$theta[j,])
      }
    }
