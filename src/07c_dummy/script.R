...
#13 <- Reduced_Data$Alpha_proportion/100
#14 <- (Reduced_Data$Delta_proportion)/100
#15] <- (Reduced_Data$Delta_AY_4_2_proportion)/100
#16] <- (Reduced_Data$Omicron_BA_1_proportion )/100
#17] <- (Reduced_Data$Omicron_BA_2_proportion)/100
#18] <- (Reduced_Data$Omicron_BA_4_proportion)/100
#19] <- (Reduced_Data$Omicron_BA_5_proportion)/100

#20 - Core_services_funding_by_weighted
#21 - Primary_care_funding_by_weighted
#22 - Specialised_services_by_weighted
#23 - unringfenced
#24 - contain_outbreak_management
#25 - ASC_infection_control_fund
...
betas_13_15 <- plot(stanfit, pars = sprintf('betas[%s]',13:19))
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
betas_13_15 <- betas_13_15 + scale_y_continuous(breaks = c(7:1),
                                              labels = c("13) Alpha", "14) Delta", "15) Delta AY4_2",
                                                         "16) Omicron BA1", "17) Omicron BA2", "18) Omicron BA4",
                                                         "19) Omicron BA5")) +
  geom_vline(xintercept = 0, color = "skyblue", lty = 5, size = 1) +ggtitle("Variant proportion coefficients")

png(file="Case_Outputs\\betas_13_19.png",
    width=1440, height=1080, res = 150)
plot(betas_13_15)
dev.off()

betas_16_21 <- plot(stanfit, pars = sprintf('betas[%s]',20:25))
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
betas_16_21 <- betas_16_21 + scale_y_continuous(breaks = c(6:1),
                                                labels = c("20) Core Services", "21) Primary Care", 
                                                           "22) Specialised Services", "23) Unringfenced",
                                                           "24) Outbreak Management", "25) ASC infection control")) +
  geom_vline(xintercept = 0, color = "skyblue", lty = 5, size = 1) +ggtitle("Funding coefficients")

png(file="Case_Outputs\\betas_20_25.png",
    width=1440, height=1080, res = 150)
plot(betas_16_21)
dev.off()
