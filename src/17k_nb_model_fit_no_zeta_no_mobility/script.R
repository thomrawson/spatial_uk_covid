...
y[,1] ~ neg_binomial_2( ((susc_scaling*susceptible_proxy[,1]).*(E[,1])).*exp(x[1] * betas + (beta_random_walk[1]) + theta), phi);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ neg_binomial_2( ((susc_scaling*susceptible_proxy[,i]).*(E[,i])).*exp( x[i] * betas + (beta_random_walk[i]) + theta), phi);  // extra noise removed removed: + theta[,i]
...
y[,1] ~ neg_binomial_2( ((susceptible_proxy[,1]).*(E[,1])).*exp(x[1] * betas + (beta_random_walk[1]) + theta), phi);  // extra noise removed removed: + theta[,i]
target += -penalty_term*fabs(beta_random_walk[1]);
for(i in 2:T){
  y[,i] ~ neg_binomial_2( ((susceptible_proxy[,i]).*(E[,i])).*exp( x[i] * betas + (beta_random_walk[i]) + theta), phi);  // extra noise removed removed: + theta[,i]
  target += -penalty_term*fabs(beta_random_walk[i]);
}
...
real log_lik[N, T]; // Log-likelihood for each data point
  ...
  for (n in 1:N) {
    for (t in 1:T) {
      log_lik[n, t] = neg_binomial_2_lpmf(y[n, t] | ((susceptible_proxy[n,t]) * (E[n,t]))*exp( dot_product(x[t,n,], betas) + beta_random_walk[t] + theta[n]), phi);
    }                                               
  }
}
"
