...
modelData <- list(N = length(Reduced_Data$Week_Cases), M = (ncol(X_dist)+ncol(X_human)),
                  ...
                  e = Reduced_Data$previous_week_cases)
...
colnames(plot_data) <- c("CODE", "Population", "Week_Cases", "Previous_Week_Cases", "u_dist_mean", "u_human_mean", "v_mean")
...
plot_grid(R_plot + theme(legend.title= element_blank()),
          ...
          nrow = 2, labels = c(sprintf("Real R est. - Week %s", Week_isolated),
