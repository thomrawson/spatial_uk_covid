...
for (n in 1:N) {
    lambda_temp[n] = log((susc_scaling * susceptible_proxy[n, i]) * (y[n, i - 1] + (zetas[n] * E_neighbours[n, i])))  + x[i,n,] * betas + (beta_random_walk[i]) + theta[n];
  }

  y[,i] ~ poisson_log(lambda_temp);  // extra noise removed removed: + theta[,i]
