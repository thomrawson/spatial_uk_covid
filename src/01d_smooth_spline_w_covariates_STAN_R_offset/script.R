...
data {
  ...
  int<lower=0> e[N, T];              // offset (previous week's cases)
...
for(t in 1:T){
for(i in 1:N){
  y[i,t] ~ poisson_log(b[t,1] + u[t][i] + v[t][i] + (X_covariates[t,i,] * betas) + log(e[i,t]));  // 
...
generated quantities {
  ...
  for(t in 1:T){
  for(i in 1:N){
  mu[t,i] = exp(b[t,1] + u[t][i] + v[t][i] + (X_covariates[t,i,] * betas) + log(e[i,t]));
  }
  }
}
"
...
plot_grid(R_est_plot + theme(legend.title= element_blank()),
          ...
          nrow = 2, labels = c(sprintf("R estimate - Week %s", Week_isolated), "GAM k40 fit",
