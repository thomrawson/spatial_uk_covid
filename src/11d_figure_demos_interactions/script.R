...
for(i in 1:T){
  if(scale_by_susceptible_pool){
    y_approx[i,,j] <- as.numeric(log((model_susc_scale*susceptible_proxy[,i])*(E[i,] + (list_of_draws$zetas[j,] *E_neighbours[,i])))) + x[i,,]%*%list_of_draws$betas[j,] + x_interaction[i,,]%*%list_of_draws$betas_interaction[j,] + (list_of_draws$beta_random_walk[j,i]) + list_of_draws$theta[j,]
    
  }else{
    y_approx[i,,j] <- as.numeric(log(susceptible_proxy[,i]*(E[i,] + (list_of_draws$zetas[j,] *E_neighbours[,i])))) + x[i,,]%*%list_of_draws$betas[j,] + x_interaction[i,,]%*%list_of_draws$betas_interaction[j,] + (list_of_draws$beta_random_walk[j,i]) + list_of_draws$theta[j,]
  }
}
...
labels_hold <- c(
  ...
  "5) Population per km^2",
  "6) Median Annual Income",
  "7) Time At Home")


#Next we have a look at how good our fit is
betas_1_10 <- stan_plot(stanfit, pars = sprintf('betas[%s]',1:7),
                        fill_color = "#e54339",
                        show_density = FALSE)
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
  betas_1_10 + scale_y_continuous(breaks = c(7:1),
                                              labels =labels_hold) +
  ...
  betas_11_13 <- stan_plot(stanfit, pars = sprintf('betas[%s]',8:10),
                          fill_color = "#e54339",
                          show_density = FALSE)
  #ci_level: 0.8 (80% intervals)
  #outer_level: 0.95 (95% intervals)
  betas_11_13 + scale_y_continuous(breaks = c(3:1),
                                  labels = c("8) Alpha Proportion", "9) Delta Proportion", "10) Omicron Proportion")) +
    geom_vline(xintercept = 0, color = "#9BC362", lty = 5, size = 1) +
  ...
  betas_14_16 <- stan_plot(stanfit, pars = sprintf('betas[%s]',11:13),
                           fill_color = "#e54339",
                           show_density = FALSE)
  #ci_level: 0.8 (80% intervals)
  #outer_level: 0.95 (95% intervals)
  ...
  betas_interaction <- stan_plot(stanfit, pars = 'betas_interaction',
                           fill_color = "#e54339",
                           show_density = FALSE)
  
  betas_interaction + scale_y_continuous(breaks = c(5:1),
  ...
  Fig3_plot <- plot_grid(betas_1_10, betas_11_13, betas_14_16, betas_interaction,
                         nrow = 3, rel_heights = c(7,3,3,5), align = "v")
