...
#######################################
#Plot posterior ranges for the beta coefficients parameter
  labels_hold <- c(
...
png(file="Outputs\\betas_11_13.png",
    width=1440, height=1080, res = 150)
plot(betas_11_13)
dev.off()
...
png(file="Outputs\\betas_14_16.png",
    width=1440, height=1080, res = 150)
plot(betas_14_16)
dev.off()
...
# ####################################################################
  ...
  ggsave(filename = "other_trajectories.png",
    ...
    other_trajectories <- rstan::traceplot(stanfit, pars=c('phi', 
                                                           'sqrtQ', 'betas'), nrow = 4)
    new_names <- c('phi', 'Q', sprintf('betas[%s]',1:16))
