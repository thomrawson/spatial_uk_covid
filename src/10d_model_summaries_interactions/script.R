...
if(scale_by_susceptible_pool){
  main_summaries <- summary(stanfit, pars = c('sqrtQ', 'susc_scaling', 'betas', 'betas_interaction', 'beta_random_walk_steps', 'zetas', 'lp__'))
  write.csv(main_summaries$summary,"Case_Outputs/main_summaries.csv", row.names = TRUE)
  
}else{
main_summaries <- summary(stanfit, pars = c('sqrtQ', 'betas', 'betas_interaction', 'beta_random_walk_steps', 'zetas', 'lp__'))
write.csv(main_summaries$summary,"Case_Outputs/main_summaries.csv", row.names = TRUE)
}
...
beta_trajectories1 <- rstan::traceplot(stanfit, pars=c('sqrtQ', sprintf('betas[%s]',1:7)), nrow = 3)
png(file="Case_Outputs\\beta_trajectories1.png",
    width=1440, height=1080, res = 150)
plot(beta_trajectories1)
dev.off()

beta_trajectories2 <- rstan::traceplot(stanfit, pars= sprintf('betas[%s]',8:13))
png(file="Case_Outputs\\beta_trajectories2.png",
    width=1440, height=1080, res = 150)
plot(beta_trajectories2)
dev.off()

interaction_trajectories <- rstan::traceplot(stanfit, pars= 'betas_interaction')
png(file="Case_Outputs\\interaction_trajectories.png",
    width=1440, height=1080, res = 150)
plot(interaction_trajectories)
dev.off()
...
#4 - IMD_Average_score
#5 - Pop_per_km2
#6 - Median_annual_income
#7 - residential_percent_change_from_baseline
#8 - Alpha_proportion
#9 - Delta_proportion
#10 - Omicron_proportion

#11 - unringfenced
#12 - contain_outbreak_management
#13 - ASC_infection_control_fund
...
#######################################
  ...
  labels_hold <- c(
    ...
    "5) Pop_per_km2", "6) Median_annual_income",
    "7) residential_movement")


beta_summaries <- main_summaries$summary
#Next we have a look at how good our fit is
betas_1_10 <- plot(stanfit, pars = sprintf('betas[%s]',1:7))
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
betas_1_10 <- betas_1_10 + scale_y_continuous(breaks = c(7:1),
                                labels = labels_hold) +
  geom_vline(xintercept = 0, color = "skyblue", lty = 5, size = 1) +ggtitle("Covariate coefficients")

png(file="Case_Outputs\\betas_1_7.png",
    width=1440, height=1080, res = 150)
plot(betas_1_10)
dev.off()


betas_11_13 <- plot(stanfit, pars = sprintf('betas[%s]',8:10))
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
betas_11_13 <- betas_11_13 + scale_y_continuous(breaks = c(3:1),
                                              labels = c("8) Alpha", "9) Delta", 
                                                         "10) Omicron")) +
  geom_vline(xintercept = 0, color = "skyblue", lty = 5, size = 1) +ggtitle("Variant proportion coefficients")

png(file="Case_Outputs\\betas_8_10.png",
    width=1440, height=1080, res = 150)
plot(betas_11_13)
dev.off()

betas_14_16 <- plot(stanfit, pars = sprintf('betas[%s]',11:13))
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
betas_14_16 <- betas_14_16 + scale_y_continuous(breaks = c(3:1),
                                                labels = c("11) Unringfenced",
                                                           "12) Outbreak Management", "13) ASC infection control")) +
...
betas_interaction <- plot(stanfit, pars = 'betas_interaction')
#ci_level: 0.8 (80% intervals)
#outer_level: 0.95 (95% intervals)
betas_interaction <- betas_interaction + scale_y_continuous(breaks = c(5:1),
                                                labels = c("IMD x mobility", "IMD x unringfenced",
                                                           "IMD x contain outbreak management", "IMD x ASC infection control",
                                                           "mobility x contain outbreak management")) +
  geom_vline(xintercept = 0, color = "skyblue", lty = 5, size = 1) +ggtitle("Interaction coefficients")

png(file="Case_Outputs\\betas_interaction.png",
    width=1440, height=1080, res = 150)
plot(betas_interaction)
dev.off()
